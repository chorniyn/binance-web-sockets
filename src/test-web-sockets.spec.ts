...
import {subscribeToFuturesTicker, subscribeToTrades} from "./subscribe-to-stream";
...
describe("Binance Websockets", () => {
    it("should subscribe to data", async () => {
        await Promise.all([
            ...
            storingIterable('BTCUSDT_futures_ticker.txt', subscribeToFuturesTicker({pairs: ['BTCUSDT'], frequencyInSeconds: 3})),
       ])
    }, 60_000);
})
